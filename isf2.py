...
rtime is in, which might include future values.
    '''
    year = rtime.year
...
We may use values from neighboring quarters and buckets.'''
    ...
    if key not in isf_est_cache:
        ...
        for q0,q1,neighbors,option in [(0,0,False,'A'),
                                       ...
                                       (-2,1,True,'D2')]:
            if debug:
                print('starting',q0,q1,neighbors,option)
            vals = []
            ...
            # is it enough?
            if len(vals) >= min_data:
                isf_est_cache[key] = round(median(vals),2), option, len(vals)
                if debug:
                    print('done option {} with {} values'.format(option,len(vals)))
                break
        if key not in isf_est_cache:
...
def test_isf_at_time(min_dt=min_rtime(),max_dt=max_rtime()):
    ...
    while dt < max_dt:
        if datetime_quarter(dt) != curr_quarter:
            ...
            st = [ stats() for b in range(0,24) ]
        # normal processing
        isf,opt,n = isf_at_time(dt, debug=False)
        isfs.append(isf)
...
''')
    ...
    print('linear: {} nonlinear: {}'.format(linear,nonlinear),file=sys.stderr)
