...
def gen_rows(conn=get_conn(),pipeIn=None,year=None):
    if pipeIn is None:
        curs = conn.cursor(MySQLdb.cursors.DictCursor) # results as Dictionaries
        if SERVER == 'hughnew':
            select = 'SELECT date_time as epoch,Basal_amt,bolus_volume,carbs FROM insulin_carb_2'
        else:
            select = 'SELECT epoch,Basal_amt,bolus_volume FROM insulin_carb'
        if year is not None:
...
def db_update(rows, tablename, keys):
    ...
    for row in rows:
        ...
        if insert_count % 1000 == 0:
            print str(insert_count)+' '
        data = list(row)
        data.append(tuple_get(row,'rec_num')) # extra occurrence for the key
